...
l2_cache=$(echo "$specs" | grep L2 | awk '{print $3}' | sed s/.$// | xargs)
...
# Note: Since l2_cache is an INTEGER data type in PSQL we do not put them in single quotes
insert_stmt="INSERT INTO host_info
VALUES(DEFAULT, '$hostname', '$cpu_number', '$cpu_architecture', '$cpu_model', '$cpu_mhz', $l2_cache,
'$total_mem', '$timestamp');"
